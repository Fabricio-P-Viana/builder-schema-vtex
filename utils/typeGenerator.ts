...
import {
  StringPropertyTypeMapper,
  EnumPropertyTypeMapper,
  ArrayPropertyTypeMapper,
  ObjectPropertyTypeMapper,
  ConditionalPropertyTypeMapper
} from './typeGenerators';
...
function mapPropertyTypeToTypeScript(prop: PropertyForm): string {
  const mappers: Record<string, () => string> = {
    string: () => new StringPropertyTypeMapper().mapToTypeScript(prop),
    number: () => 'number',
    boolean: () => 'boolean',
    object: () => new ObjectPropertyTypeMapper().mapToTypeScript(prop),
    array: () => new ArrayPropertyTypeMapper().mapToTypeScript(prop),
    enum: () => new EnumPropertyTypeMapper().mapToTypeScript(prop),
    conditional: () => new ConditionalPropertyTypeMapper().mapToTypeScript(),
  };

  const mapper = mappers[prop.type];
  return mapper ? mapper() : 'any';
}
